...
class PancakeActivity : Activity() {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    btnBrowse?.setOnClickListener {
      showFolderSelector { selectedPath ->
    ...
    btnStart?.setOnClickListener {
      try {
        ...
        statusText?.text = "Serveur démarré sur $localIP:$port"
        statusText?.setTextColor(0xFF66FF66.toInt())
    ...
    btnStop?.setOnClickListener {
      ftpServer?.stop()
      ftpServer = null
      statusText?.text = "Serveur arrêté"
  ...
  private fun startFtpServerAutomatically(
    ...
    try {
      ...
      statusText?.text = "Erreur de démarrage automatique: ${e.message}"
      statusText?.setTextColor(0xFFFF6666.toInt())
